...
`))}import A from"path";function O(e,t){let n=e.trim(),i=A.resolve(t.trim());return A.join(i,n)}import{promisify as Pe}from"node:util";import we from"chalk";import _e from"ora";import{exec as Ne}from"node:child_process";function C(e){return o(this,null,function*(){let t=Pe(Ne),n=_e({text:"Forging a new Expo app...",color:"cyan"}).start();try{yield t(`npx create-expo-app@latest "${e}" --template blank-typescript`),n.succeed(`Expo app created successfully!
...
export default [
  ...
  `;import{exec as Oe}from"node:child_process";function l(e,t){return new Promise((n,i)=>{console.log(`Executing command: ${e} in ${t}`),Oe(e,{cwd:t},(s,p,f)=>{if(s){i(`Error executing command: ${f||s.message}`);return}n(p)})})}function U(e){return o(this,null,function*(){let t=L.resolve(e,"eslint.config.mjs");yield c("Setting up ESLint",[{text:"Installing ESLint dependencies AIEIAIEA ",action:()=>o(this,null,function*(){console.log("ici")})},{text:"Installing ESLint dependencies",action:()=>o(this,null,function*(){let i=`npm install --save-dev ${I}`;yield l(i,e)})},{text:"Creating ESLint configuration",action:()=>o(this,null,function*(){T.writeFileSync(t,V,"utf-8")})},{text:"Running ESLint with --fix",action:()=>o(this,null,function*(){yield l("npx eslint . --fix",e)})}])})}var W=`
# dependencies
node_modules/
